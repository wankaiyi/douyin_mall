...
type Message struct {
	...
	Content  string   `gorm:"not null; type:varchar(255);comment:'消息内容'"`
...
func ConversionExist(db *gorm.DB, ctx context.Context, uuid string) (bool, error) {
	var count int64
	result := db.WithContext(ctx).Model(&Message{}).Where(&Message{Uuid: uuid}).Count(&count)
	if result.Error == nil {
		return count > 0, nil
	}
